...
Function GetChartSize(ByVal helmetType As String) As Variant
    ' デバッグ情報を表示
    Debug.Print "GetChartSize received value: [" & helmetType & "]"
    Debug.Print "Value type: " & TypeName(helmetType)
    Debug.Print "Length: " & Len(helmetType)
    Debug.Print "Contains BASEBALL: " & (InStr(1, helmetType, "BASEBALL", vbTextCompare) > 0)
    
    Dim size(1) As Long
    
    If InStr(1, helmetType, "BASEBALL", vbTextCompare) > 0 Then
        size(0) = 270  ' Width
        size(1) = 300  ' Height
        Debug.Print "Selected BASEBALL size"
    Else
        size(0) = 350  ' Width
        size(1) = 300  ' Height
        Debug.Print "Selected DEFAULT size"
    End If
