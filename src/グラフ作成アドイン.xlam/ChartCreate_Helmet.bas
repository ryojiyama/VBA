...
Sub CreateGraphHelmet(userInput As String)
    Dim ws As Worksheet
    Set ws = ActiveWorkbook.Sheets("LOG_Helmet")
    Dim lastRow As Long
...
Function GetChartSize(ByVal graphType As String) As Variant
    ...
    Select Case graphType
        ...
        Case "その他"
            ...
            size(1) = 500  ' Height
        Case Else
            size(0) = 500  ' Default Width
            size(1) = 300  ' Default Height
    End Select
...
Sub UpdateMaxValueInRow(ByRef ws As Worksheet, ByVal row As Long)
  'InspectHelmetDurationTime()内のプロシージャ_行内の最大値を更新し、最大値を記録した時刻のセルに色をつける。
    ...
    ' 行内の範囲をセットする。
    Set rng = ws.Range(ws.Cells(row, "AA"), ws.Cells(row, ws.columns.Count).End(xlToLeft))
    ' 最大値を取得する。
    ...
    If maxValueColumn <> 0 Then
        ws.Cells(row, "I").value = ws.Cells(1, "AA").Offset(0, maxValueColumn - 1).value
    End If
End Sub
...
Sub TestCreateChartID()
    Dim ws As Worksheet
    Set ws = ActiveWorkbook.Sheets("LOG_Helmet")  ' テストするワークシート名を指定
