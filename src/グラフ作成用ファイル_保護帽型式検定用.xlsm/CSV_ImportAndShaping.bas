...
Sub ImportCSVsAndSortSheets()
    ...
    Dim DEFAULT_SHEET_ORDER As Variant '配列のサイズを指定
    DEFAULT_SHEET_ORDER = Array("LOG_Helmet", "Setting", "Hel_SpecSheet", "Penetration", "Imapct_Top", "Impact_Front", "Impact_Back", "Imapct_Side")
    ...
    'Debug.Print "検索パス: " & myPath & "*.csv"
    myFile = Dir(myPath & "*.csv")
    Debug.Print "最初のファイル: " & myFile
...
Sub SortSheetsByOrder(ByRef wb As Workbook, ByVal sheetNames As Collection, ByVal defaultOrder As Variant)
    ...
    For i = LBound(defaultOrder) To UBound(defaultOrder)
        If SheetExists(CStr(defaultOrder(i)), wb) Then
            If i + 1 <= wb.Sheets.Count Then
                Sheets(CStr(defaultOrder(i))).Move Before:=Sheets(i + 1)
            Else
                Sheets(CStr(defaultOrder(i))).Move After:=Sheets(wb.Sheets.Count)
            End If
        Else
            Debug.Print "Sheet '" & defaultOrder(i) & "' does not exist."
        End If
    Next i
...
Sub ImportCSVsAndSortSheets_0926Before()
    ...
    Set wb = ThisWorkbook
    myPath = ThisWorkbook.path & "CSV"   ' Path changed to subfolder 'CSV'V'
    ...
    Application.ScreenUpdating = True
End Sub
Sub SortSheetsByOrder_0415Before(ByRef wb As Workbook, ByVal sheetNames As Collection, ByVal defaultOrder As Variant)
    ' Debug
