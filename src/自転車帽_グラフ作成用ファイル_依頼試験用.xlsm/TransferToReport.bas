...
Private Function GetMappingDictionary() As Object
    ...
    dict.Add "P", "F" ' 転記元のM列 → 転記先のF列
    dict.Add "O", "G" ' 転記元のN列 → 転記先のG列
    dict.Add "T", "H" ' 転記元のN列 → 転記先のH列
    ' 必要に応じて他の列のマッピングも追加
...
Private Sub TransferMappedValues(wsSource As Worksheet, wsDest As Worksheet, sourceRow As Long, destRow As Long, mappingDict As Object)
    ' TransferDataWithMappingAndFormattingのサブプロシージャ。マッピングに基づいて値を転記する
    Dim key As Variant
...
Private Sub ApplyFormattingToNewRows(ws As Worksheet, startRow As Long, endRow As Long)
    ...
    For currentRow = startRow To endRow
        ' 現在の行の範囲を取得
        Set targetRange = ws.Range("B" & currentRow & ":H" & currentRow)
        ...
        Set iRange = ws.Range("J" & currentRow)
...
Sub InsertTextToReport()
    ...
    ' J列に"Insert"が含まれる行を探索し、H列に"開"が含まれているか確認
    For i = 1 To lastRow
        If InStr(ws.Cells(i, "J").value, "Insert") > 0 Then
