...
Sub TransferDataBasedOnID()
    ...
    For i = 1 To lastRow
        ...
        dummyHead = wsSource.Range("Q" & i).value
        anvilType = wsSource.Range("P" & i).value
        preProcess = wsSource.Range("M" & i).value
        addNotes = wsSource.Range("V" & i).value
        tempArray = Array( _
            idParts(0), _
            targetSheetName, _
            maxValue, _
...
' データを各シートに転記するTransferDataBasedOnIDのサブプロシージャ
    ...
    For Each dataItem In data
        ...
        wsDest.Range("H" & nextRow).value = addNotes
    Next dataItem
End Sub
